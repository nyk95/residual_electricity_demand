...
for reg in reg_list:
    ...
    file_classic_est = 'Data/Hindcast output/'+reg+' Hindcast_classic_electricity_demand_est.csv'
    file_entire = 'Data/Hindcast output/'+reg+' Hindcast_entire_electricity_demand.csv'
    if reg == 'MT':
        file_classic_est = 'Data/Hindcast output/IT Hindcast_classic_electricity_demand_est.csv'
        file_entire = 'Data/Hindcast output/IT Hindcast_entire_electricity_demand.csv'
    ...
    #ax[1].fill_between(df.resample('m').mean().index.values,df['Demand'].resample('m').quantile(0.25)/1e3,df['Demand'].resample('m').quantile(0.75)/1e3,color='black',alpha=0.3)
    #ax[1].fill_between(df.resample('m').mean().index.values,df['Demand_classic_est'].resample('m').quantile(0.25)/1e3,df['Demand_classic_est'].resample('m').quantile(0.75)/1e3,color='blue',alpha=0.3)
    #ax[1].fill_between(df.resample('m').mean().index.values,df['Demand_classic_adj_rem'].resample('m').quantile(0.25)/1e3,df['Demand_classic_adj_rem'].resample('m').quantile(0.75)/1e3,color='red',alpha=0.3)
    ...
    plt.suptitle(reg)
    check_if_dir_exists('output/simulation_overview_plots')
    plt.savefig('output/simulation_overview_plots/'+reg+'.png')
    
    df_final = df_final.join(df['Demand_classic_adj_rem'].rename(reg),how='left')
    
    
df_final.to_csv('output/residual_electricity_demand.csv')
